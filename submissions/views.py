...
class ClassroomOwnerMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        messages.error(self.request, 'You do not have permission to modify this classroom.')
...
class SubmissionAccessMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        messages.error(self.request, 'You do not have permission to view this submission.')
        return redirect('dashboard')
...
class SubmissionEditMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        if not self.get_object().is_draft:
            ...
            messages.error(self.request, 'You do not have permission to edit this submission.')
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_teacher:
            ...
            classrooms = Classroom.objects.for_teacher(user).annotate(
                ...
                pending_count=Count('submissions', filter=Q(
                    submissions__status=ProjectSubmission.Status.SUBMITTED,
            ...
            context['pending_submissions'] = ProjectSubmission.objects.for_teacher(user).filter(
            ...
            context['draft_count'] = submissions.filter(status=ProjectSubmission.Status.DRAFT).count()
            context['submitted_count'] = submissions.filter(status=ProjectSubmission.Status.SUBMITTED).count()
            context['graded_count'] = submissions.exclude(grade__isnull=True).count()
            
            # Calculate average grade
            avg_grade = submissions.exclude(grade__isnull=True).aggregate(avg=Avg('grade'))
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_teacher and classroom.teacher == user:
            ...
            context['submissions'] = ProjectSubmission.objects.filter(
                classroom=classroom
            ).select_related('created_by').prefetch_related('collaborators')[:10]
        else:
...
class RegenerateJoinCodeView(LoginRequiredMixin, ClassroomOwnerMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'New join code generated: {self.object.join_code}')
...
class SubmissionDetailView(LoginRequiredMixin, SubmissionAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['is_collaborator'] = submission.collaborators.filter(pk=user.pk).exists()
...
class SubmissionCreateView(LoginRequiredMixin, StudentRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.classroom.is_student_member(request.user):
            messages.error(request, 'You must be a member of this classroom to submit a project.')
        ...
        if ProjectSubmission.objects.filter(
            ...
            messages.error(request, 'You already have a submission in this classroom.')
            return redirect('classroom_detail', pk=self.classroom.pk)
...
class SubmissionSubmitView(LoginRequiredMixin, SubmissionEditMixin, FormView):
    ...
    def form_valid(self, form):
        if self.submission.submit():
            messages.success(self.request, 'Project submitted successfully! It is now visible to your teacher.')
        else:
            messages.error(self.request, 'Unable to submit project. It may have already been submitted.')
...
class TeacherSubmissionListView(LoginRequiredMixin, TeacherRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Apply filters
        self.filter_form = SubmissionFilterForm(self.request.GET, user=self.request.user)
        queryset = self.filter_form.filter_queryset(queryset)
    ...
    def get_context_data(self, **kwargs):
        ...
        # Statistics
        all_submissions = ProjectSubmission.objects.for_teacher(self.request.user)
        context['total_submissions'] = all_submissions.count()
        context['pending_count'] = all_submissions.filter(
            status=ProjectSubmission.Status.SUBMITTED,
            grade__isnull=True
        ).count()
        context['graded_count'] = all_submissions.exclude(grade__isnull=True).count()
...
class GradeSubmissionView(LoginRequiredMixin, TeacherRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if submission.classroom.teacher != request.user:
            messages.error(request, 'You can only grade submissions in your own classrooms.')
